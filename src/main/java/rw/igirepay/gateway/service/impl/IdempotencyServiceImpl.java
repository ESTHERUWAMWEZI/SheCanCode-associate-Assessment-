 ...
 *   1. First request          → process, cache, return (cacheHit=false)
 *   2. Retry (same payload)   → return cached future result (cacheHit=true)
 *   3. Conflict (diff payload)→ throw ConflictingIdempotencyKeyException → 409
 */
@Slf4j
@Service
...
public class IdempotencyServiceImpl implements IdempotencyService {
    ...
    public IdempotencyResult processWithIdempotency(String idempotencyKey, PaymentRequest request) {
        ...
        if (existing == null) {
            return processFirstRequest(candidate, request, idempotencyKey);
        }

        return handleExistingRecord(existing, requestHash, idempotencyKey);
    }
    ...
    private IdempotencyResult processFirstRequest(
            IdempotencyRecord record, PaymentRequest request, String idempotencyKey) {
